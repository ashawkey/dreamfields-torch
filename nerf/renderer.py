...
class NeRFRenderer(nn.Module):
    ...
    def run(self, rays_o, rays_d, bound, num_steps, upsample_steps, perturb):
    ...
    def run_cuda(self, rays_o, rays_d, bound, num_steps, upsample_steps, perturb):
        # rays_o, rays_d: [B, N, 3], assumes B == 1
        # return: image: [B, N, 3], depth: [B, N]
        ...
        if self.training:
            ...
            # we need to render the whole image, so must force_all_rays !!!
            xyzs, dirs, deltas, rays = raymarching.march_rays_train(rays_o, rays_d, bound, self.density_grid, self.mean_density, near, far, counter, self.mean_count, perturb, 128, True)
            sigmas, rgbs = self(xyzs, dirs, bound=bound)
            # print('rays numsteps', rays[:, 2].sum().item())
            # print('sigmas', sigmas.shape, sigmas.dtype, sigmas.min().item(), sigmas.max().item())
            # print('rgbs', rgbs.shape, rgbs.dtype, rgbs.min().item(), rgbs.max().item())
            ...
            # rectify depth (shade_kernel_nerf)
            depth = torch.clamp(depth - near, min=0) / (far - near)


        weights_sum = weights_sum.reshape(B, N)
        #depth = depth.reshape(B, N) # do not need depth for now...
        image = image.reshape(B, N, 3)

        # valid pixel mask
        mask = (near < far).reshape(B, N)

        # TODO: origin tracking (only valid for training)
    ...
    def render(self, rays_o, rays_d, bound=1, num_steps=128, upsample_steps=128, staged=False, max_ray_batch=4096, perturb=False, **kwargs):
        # rays_o, rays_d: [B, N, 3], assumes B == 1
        ...
        if staged and not self.cuda_ray:
            ...
            for b in range(B):
                ...
                while head < N:
                    ...
                    weights_sum_, image_ = _run(rays_o[b:b+1, head:tail], rays_d[b:b+1, head:tail], bound, num_steps, upsample_steps, perturb)
                    weights_sum[b:b+1, head:tail] = weights_sum_
                    image[b:b+1, head:tail] = image_
                    head += max_ray_batch

            results = {}
            results['weights_sum'] = weights_sum # weights_sum
            results['rgb'] = image

        else:
            results = _run(rays_o, rays_d, bound, num_steps, upsample_steps, perturb)
